...
checkAndGetPoolingParameters(OpTy op, ConversionPatternRewriter &rewriter,
  ...
  if (matchPattern(op.ceil_mode(), m_TorchConstantBool(&ceilMode))) {
    if (ceilMode) {
      return rewriter.notifyMatchFailure(op,
                                         "only ceil_mode false is supported");
    }
  } else
    return rewriter.notifyMatchFailure(op,
...
template <typename OpTy>
static LogicalResult
computeMaxPool2d(OpTy op, ConversionPatternRewriter &rewriter, Value self,
                 SmallVector<int64_t, 2> kernelSizeInts,
                 SmallVector<int64_t, 2> strideInts,
                 SmallVector<int64_t, 2> paddingInts,
                 SmallVector<int64_t, 2> dilationInts, Value &result) {
...
public:
  ...
  matchAndRewrite(AtenMaxPool2dOp op, OpAdaptor adaptor,
    ...
    if (failed(computeMaxPool2d<AtenMaxPool2dOp>(
            op, rewriter, self, kernelSizeInts, strideInts, paddingInts,
            dilationInts, maxPool2d)))
...
public:
  ...
  matchAndRewrite(AtenMaxPool2dWithIndicesOp op, OpAdaptor adaptor,
    ...
    Value maxPool2d;
    if (failed(computeMaxPool2d<AtenMaxPool2dWithIndicesOp>(
            op, rewriter, self, kernelSizeInts, strideInts, paddingInts,
            dilationInts, maxPool2d)))
      return rewriter.notifyMatchFailure(op, "unable to compute maxpool2d");
    ...
    Value indicesTensor = createZeroInitTensor(
        rewriter, loc, resultShape, indicesRankedTensorType.getElementType());
    ...
    SmallVector<AffineMap> indexingMaps = {
        AffineMap::get(6, 0, inputExprs, op.getContext()),
        AffineMap::get(6, 0, kernelExprs, op.getContext()),
        AffineMap::get(6, 0, outputExprs, op.getContext())};
    SmallVector<StringRef> iteratorTypes(6, getParallelIteratorTypeName());
            ...
            .create<linalg::GenericOp>(
                ...
                [&](OpBuilder &b, Location loc, ValueRange args) {
                  ...
                  Value result = b.create<arith::SelectOp>(
                      loc, pred, castIndexToInt(b, loc, outIndex), args[2]);
